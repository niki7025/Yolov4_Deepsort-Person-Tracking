...
RUN OPENBLAS_CORETYPE=ARMV8 python3.7 -m pip install -U \
    pip \
    setuptools \
    wheel && \
    pip3 install \
...
WORKDIR ../
RUN OPENBLAS_CORETYPE=ARMV8 python3.7 object_tracker.py --pictures_path ./data/pictures/ --output ./outputs/tracker.avi --model yolov4 --dont_show --info
