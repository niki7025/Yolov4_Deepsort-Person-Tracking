...
def cspdarknet53(input_data):
    ...
    input_data = trt.concat([input_data, route], axis=-1)
    ...
    input_data = trt.concat([trt.nn.max_pool(input_data, ksize=13, padding='SAME', strides=1), trt.nn.max_pool(input_data, ksize=9, padding='SAME', strides=1)
                            , trt.nn.max_pool(input_data, ksize=5, padding='SAME', strides=1), input_data], axis=-1)
...
def cspdarknet53_tiny(input_data):
    ...
    route_1 = input_data
    input_data = common.convolutional(input_data, (3, 3, 32, 32))
    input_data = trt.concat([input_data, route_1], axis=-1)
    input_data = common.convolutional(input_data, (1, 1, 32, 64))
    input_data = trt.concat([route, input_data], axis=-1)
    input_data = trt.keras.layers.MaxPool2D(2, 2, 'same')(input_data)
...
def darknet53_tiny(input_data):
    ...
    input_data = trt.keras.layers.MaxPool2D(2, 1, 'same')(input_data)
    input_data = common.convolutional(input_data, (3, 3, 512, 1024))
